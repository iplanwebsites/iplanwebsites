![GitHub contributions](https://github-summary.cookskill.dev/demo?v=4&theme=light)
### Hi there! I'm Félix 👋

- 📚 **I teach at [UQAM School of Media](https://edm.uqam.ca/)**
- 🔭 **Compulsive creator of [random projects](https://felixmenard.com/)** 



- 📫 **Reach Out:** Whether it's a potential collaboration, a project idea, or you simply want to chat:
  - 🐦 [Follow me on Twitter](https://twitter.com/felix_m) for snippets of wisdom and updates.
  - 💼 [Connect on LinkedIn](https://www.linkedin.com/in/menard) for professional engagements.
  - 📧 Drop an email at hello @ felixmenard.com we could start something exciting.
    

 

[![My Skills](https://go-skill-icons.vercel.app/api/icons?i=cloudflare,ts,js,nodejs,python,postgresql,redis,mongodb,vue,react,electron,tensorflow,nextjs,express,html,css,notion,npm,vercel,git,github,vscode,claude,chatgpt)](https://go-skill-icons.vercel.app)  
